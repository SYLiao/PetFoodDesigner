...
const IngredientFormPage = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="ingredient-form">
      {Array.from({ length: numIngredients }).map((_, index) => {
        ...
        return (
          <div key={index} className="form-group">
            ...
            {index > 2 && (
              <button type="button" className="delete-button" onClick={() => handleRemoveIngredient(index)}>
      ...
      <button type="button" onClick={() => setNumIngredients(numIngredients + 1)} className="add-button">
